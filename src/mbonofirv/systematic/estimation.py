...
def load_historical_data(
    db_path="./db/MBONOdata.db", generate_ql_objects=True, print_head=False
):
    """
    ...
    if generate_ql_objects is True:
        bonds = define_ql_bonds_bulk(
            list(df["BBGID"].values), id_type="BBGID", db_path=db_path
        )
        df["QL bond"] = bonds
    df.set_index("BBGID", inplace=True)
    query = """SELECT InstrumentID,  Date, Price FROM MarketData ;"""
    agg_price_data = pd.read_sql_query(query, con, parse_dates=["Date"]).sort_values(
        ["Date"]
    )
    price_data = agg_price_data.pivot_table(
        index="Date", columns="InstrumentID", values="Price"
    )
    price_data.columns = [
        df.index[df["InstrumentID"] == c][0] for c in price_data.columns
    ]
    query = """SELECT InstrumentID,  Date, Residual FROM FitData ;"""
    agg_fit_data = pd.read_sql_query(query, con, parse_dates=["Date"]).sort_values(
        ["Date"]
    )
    fit_data = agg_fit_data.pivot_table(
        index="Date", columns="InstrumentID", values="Residual"
    )
...
def generate_triplet_signals(
    ...
    for i in tqdm(range(lookback_years - 1, len(eoy), 1)):
        idx = pd.bdate_range(
            start=eoy[i] + pd.tseries.offsets.DateOffset(years=-lookback_years),
            end=eoy[i],
        )
        ...
        for j, t1 in enumerate(tenors):
            for k, t2 in enumerate(tenors[j + 1 : j + 2]):
                for t3 in tenors[j + k + 2 : j + k + 3]:
                    pca_bf_weights = get_pca_bf_weights(
                        df_zc.loc[train_idx, [t1, t2, t3]]
                    )
                    ...
                    if adfuller(pca_bf_ts)[1] < p_val_crit:
                        ...
                        if (hl < hl_crit) & (sigma_eq > sigma_crit):
                            print(
                                test_idx[0], t1, t2, t3, int(hl), int(10000 * sigma_eq)
                            )
                            pca_bf_test = df_zc.loc[test_idx, [t1, t2, t3]].dot(
                                pca_bf_weights
                            )
                            pca_strategy = MeanReversionSignal(
                                [t1, t2, t3], pca_bf_weights, pca_bf_test, mu, sigma_eq
                            )
                            pca_strategy.set_trading_rule(
    ...
    for i in tqdm(range(lookback_years - 1, len(eoy), 1)):
        ...
        for j, t1 in enumerate(tenors):
            for k, t2 in enumerate(tenors[j + 1 : j + 5]):
                pca_spread_weights = get_pca_spread_weights(
                    df_zc.loc[train_idx, [t1, t2]]
                )
                ...
                if adfuller(pca_spread_ts)[1] < p_val_crit:
                    ...
                    if (hl < hl_crit) & (sigma_eq > sigma_crit):
                        ...
                        pca_spread_test = df_zc.loc[test_idx, [t1, t2]].dot(
                            pca_spread_weights
                        )
                        strategy = MeanReversionSignal(
                            [t1, t2], pca_spread_weights, pca_spread_test, mu, sigma_eq
                        )
                        strategy.set_trading_rule(
                            ol=z_ol, cl=z_cl, os=z_os, cs=z_cs, ts=30
                        )
...
def plot_stats(total_pnl, trading):
    ...
    for i in range(2):
        df_stats.iloc[0, i] = (
            (trading.position * trading.dirty_prices / 100).abs().sum(axis=1).mean()
        )
        ...
        df_stats.iloc[4, i] = (trading.trades * trading.dirty_prices).abs().sum(
            axis=1
        ).sum() / (trading.position * trading.dirty_prices).abs().sum(axis=1).sum()
        df_stats.iloc[5, i] = df_stats.iloc[1, i] / df_stats.iloc[2, i]
        df_stats.iloc[6, i] = (
            df_stats.iloc[1, i]
            / np.sqrt(np.sum(total_pnl[i][total_pnl[i] < 0] ** 2) / len(total_pnl[i]))
            / np.sqrt(261)
        )
        df_stats.iloc[7, i] = (
            total_pnl[i].cumsum() - total_pnl[i].cumsum().cummax()
        ).min()
        df_stats.iloc[8, i] = total_pnl[i].quantile(0.05)
    net_pnl = total_pnl[1]
    rolling_sharpe = np.sqrt(261) * (
        net_pnl.rolling(261).mean() / net_pnl.rolling(261).std()
    )
    fig, ax = plt.subplots(
        4, 1, figsize=(20, 20), gridspec_kw={"height_ratios": [2, 1, 1, 1]}, sharex=True
    )
