...
class MeanReversionSignal:
    ...
    def estimate_target_risk(self):
        ...
        discrete_signal = discretise_signal(
            signal, self.open_long, self.close_long, self.open_short, self.close_short
        )
        target_risk = np.repeat(
            self.weights.reshape(1, -1), len(signal), axis=0
        ) * discrete_signal.to_numpy().reshape(-1, 1)
        target_risk = pd.DataFrame(
            data=target_risk, index=self.time_series.index, columns=self.tenors
        )
...
class AggregatedSignal:
    ...
    def estimate_target_risk(self):
        ...
        for i in range(1, len(self.strategies)):
            ...
            target_risk = target_risk.add(
                self.strategies[i].target_risk * self.weights[i], fill_value=0
            )
    ...
    def set_target_volatility(self, target_volatility, zc_curves):
        ...
        for d in tqdm(self.target_risk.index):
            ...
            rolling_history = (
                zc_curves.diff().loc[idx, self.target_risk.columns].to_numpy()
            )
            cov = np.cov(rolling_history.T)
            vol = np.sqrt(261) * np.sqrt(
                np.dot(
                    np.dot(self.target_risk.loc[d].to_numpy(), cov),
                    self.target_risk.loc[d].to_numpy().T,
                )
            )
            if vol != 0:
                self.target_risk.loc[d] = (
                    self.target_risk.loc[d] * target_volatility / vol
                )
        return self
