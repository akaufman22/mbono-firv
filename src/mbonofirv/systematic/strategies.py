...
class SystematicStrategy:
    ...
    def get_ql_obj(self, date):
        ...
        spot_dates = [
            ql_date + ql.Period(int(t), ql.Years) for t in ([0] + self.tenors_years)
        ]
    ...
    def rebalance_order(self, position, target_risk, date, spot_curve_handle):
        ...
        if self.rebal_rule == "vol":
            risk_dev = np.sqrt(261) * np.sqrt(
                np.dot(
                    np.dot(tracking_error.to_numpy(), cov), tracking_error.to_numpy().T
                )
            )
    ...
    def calc_position(self, min_lot=0):
        ...
        target_risk = pd.DataFrame(
            columns=self.tenors, index=strategy_risk.index, data=0
        )
        target_risk.loc[strategy_risk.index, strategy_risk.columns] = strategy_risk
        df_position = pd.DataFrame(
            index=target_risk.index, columns=self.instrument_universe.index, data=0
        )
        df_risk = pd.DataFrame(
            index=target_risk.index, columns=target_risk.columns, data=0
        )
        ...
        df_trades.loc[d, target_order.index] = target_order * (
            target_order.abs() > min_lot
        )
        ...
        for i in tqdm(range(1, len(target_risk.index))):
            ...
            target_order = self.rebalance_order(
                df_position.loc[d], target_risk.loc[d_signal], d, spot_curve_handle
            )
            df_trades.loc[d, target_order.index] = target_order * (
                target_order.abs() > min_lot
            )
    ...
    def calc_total_pnl(self, tcosts=0):
        ...
        sett_dates = list(
            self.position.reset_index()[index_name]
            .apply(ql.Date().from_date)
            .map(lambda x: self.calendar.advance(x, self.settlement_days, ql.Days))
        )
        df_position_nonzero = self.position.loc[
            :, (self.position != 0).any(axis=0)
        ].copy()
        df_settlement = pd.DataFrame(
            index=self.position.index, columns=["Settle Date"], data=sett_dates
        )
        ...
        df_dirty_prices = pd.DataFrame(
            index=self.position.index, columns=self.market_data.columns, data=0
        )
        df_tcosts = pd.DataFrame(
            index=self.trades.index, columns=self.market_data.columns, data=0
        )
        df_coupons = pd.DataFrame(
            index=self.position.index, columns=self.position.columns, data=0
        )
        for b in df_position_nonzero.columns:
            ...
            for d in self.position.index:
                ...
                if ql_d_settle in coupon_dates:
                    df_coupons.loc[d, b] = coupon_amounts[
                        coupon_dates.index(ql_d_settle)
                    ]
                df_dirty_prices.loc[d, b] = self.market_data.loc[
                    d, b
                ] + bond.accruedAmount(
                    self.calendar.advance(ql_d, self.settlement_days, ql.Days)
                )
                try:
                    df_tcosts.loc[d, b] = self.market_data.loc[d, b] - bond.cleanPrice(
                        bond.bondYield(
                            self.market_data.loc[d, b],
                            self.day_count,
                            self.yield_basis,
                            ql.Compounded,
                        )
        ...
        self.funding = (
            -(df_sett_position * df_dirty_prices / 100)
            .mul(repo_rates, axis=0)
            .mul(df_settlement["Funding Days"], axis=0)
            / 360
        )
        self.tcosts = -df_tcosts * self.trades.abs() / 100
        self.pnl = (
            (self.position.shift(1) * df_dirty_prices.diff() / 100)
            + self.funding
            + self.coupons
            + self.tcosts
        )
...
def trading_order(
    ...
    for i in sensitivities.index:
        ...
        sensitivities.loc[i, curve_tenors] = bond_sensitivities(
            bond, date, spot_curve_handle, curve_tenors_years
        )
    ...
    if fit_data is not None:
        ...
        for t in curve_tenors:
            ...
            mask = (
                abs((sensitivities["Maturity"] - tenor_date).dt.days)
                < maturity_band * years * 365
            )
            ...
            if len(candidates) == 0:
                ...
                benchmarks.append(
                    (np.sign(risk_to_trade[t]) * fit_data[candidates]).idxmin()
                )
    ...
    order = pd.Series(
        ...
        data=100
        * np.linalg.lstsq(
            sensitivities.loc[benchmarks, curve_tenors].to_numpy().T,
            risk.to_numpy(),
            rcond=None,
        )[0],
    )
...
def portfolio_risk(instruments, date, portfolio, spot_curve_handle, tenors_years):
    ...
    for i in portfolio.index:
        ...
        risk += (
            bond_sensitivities(bond, date, spot_curve_handle, tenors_years)
            * portfolio.loc[i]
            / 100
        )
    return risk
...
def bond_sensitivities(bond, date, spot_curve_handle, tenors_years):
    ...
    spreaded_yts = ql.YieldTermStructureHandle(
        ql.SpreadedLinearZeroInterpolatedTermStructure(
            spot_curve_handle, spreads, spot_dates
        )
    )
