...
def get_latest_curve_data():
    ...
    curve_data = pd.read_sql_query(
        query, con, parse_dates=["Date"], index_col=["Date"]
    ).sort_index()
    return curve_data
...
def model_residuals(model, vector, k):
    ...
    reduced_w = np.append(full_w[:k], np.zeros([dimensions - k, dimensions])).reshape(
        dimensions, dimensions
    )
    residuals = (
        vector
        - model.mean_
        - np.dot(np.dot(vector - model.mean_, reduced_w.T), reduced_w)
    )
    return residuals
