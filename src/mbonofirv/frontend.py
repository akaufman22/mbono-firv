...
models = pd.DataFrame(
    index=["Belly", "SE", "LE"],
    columns=["Model", "Tenors", "TenorsStr", "PC1", "PC2", "PC3"],
)
...
fit_table = LiveFit.fit.sort_values(["Maturity"])[
    ["Name", "Maturity", "Price", "Yield", "Th Price", "Th Yield", "Difference"]
].copy()
fit_table["Z-score"] = get_fit_zscores(LiveFit.fit["Difference"])
fit_table["Difference"] = 10000 * fit_table["Difference"]
curve_table = pd.DataFrame(
    index=["Rate", "Belly PC1", "Belly PC2", "SE PC1", "SE PC2", "LE PC1", "LE PC2"],
...
for m in models.index:
    ...
    curve_table.loc[m + " PC1", cols] = model_residuals(model, zero_curve[cols], 1)
    curve_table.loc[m + " PC2", cols] = model_residuals(model, zero_curve[cols], 2)
...
pc_series = pd.read_sql_query(query, con, parse_dates=["Date"], index_col="Date")

components_stats = pd.DataFrame(
    index=[m + " " + c for m in models.index for c in ["PC1", "PC2", "PC3"]],
...
for c in ["PC1", "PC2", "PC3"]:
    for m in models.index:
        ...
        components_stats.loc[m + " " + c, "Zsc"] = (
            models.loc[m, c] - time_series.mean()
        ) / time_series.std()
        components_stats.loc[m + " " + c, "ADF p-val"] = adfuller(time_series)[1]
        components_stats.loc[m + " " + c, "HE"] = hurst_exponent(time_series.values)
            ...
            buttons=list(
                [
                    dict(
                        label="All",
                        method="update",
                        args=[{"visible": [True] * 6}, {"title": "1st PC"}],
                    ),
                    dict(
                        label="Belly",
                        method="update",
                        args=[
                            {"visible": [True] * 2 + [False] * 4},
                            {"title": "1st PC Belly"},
                        ],
                    ),
                    dict(
                        ...
                        method="update",
                        args=[
                            {"visible": [False] * 2 + [True] * 2 + [False] * 2},
            ...
            buttons=list(
                [
                    dict(
                        label="All",
                        method="update",
                        args=[{"visible": [True] * 6}, {"title": "2nd PC"}],
                    ),
                    dict(
                        label="Belly",
                        method="update",
            ...
            buttons=list(
                [
                    dict(
                        label="All",
                        method="update",
                        args=[{"visible": [True] * 6}, {"title": "3rd PC"}],
                    ),
                    dict(
                        label="Belly",
                        method="update",
...
app.layout = html.Div(
    children=[
        ...
        html.Div(
            [
                html.H4("Individual Issues Rich/Cheap"),
                dash_table.DataTable(
                    id="live-market-table",
                                ...
                                html.Div(
                                    [
                                        dash_table.DataTable(
                                            ...
                                            data=curve_table.reset_index().to_dict(
                                                orient="records"
                                            ),
                                        )
                                    ]
                                ...
                                html.Div(
                                    [
                                        ...
                                        dash_table.DataTable(
                                            ...
                                            data=components_stats.reset_index().to_dict(
                                                orient="records"
                                            ),
                                            style_data_conditional=[
                                                {
