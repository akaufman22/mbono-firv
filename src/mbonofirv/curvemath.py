...
def define_ql_bonds(instrument_ids, id_type='BBGID'):
    ...
    for i in instrument_ids:
        bond_schedule = pd.read_sql_query(query, con, parse_dates=['Date'],
                                          params=(i,))
...
def fitted_ql_curve(bonds_sample, prices,
    ...
    for i, b in enumerate(bonds_sample):
        bond_helpers.append(ql.BondHelper(ql.QuoteHandle(
            ql.SimpleQuote(prices[i])), b))
    yield_curve_fit = ql.FittedBondDiscountCurve(settlement_date, bond_helpers,
                                                 DAY_COUNT, curve_fitting)
...
def mbono_market_snapshot(df_input, valuation_date=ql.Date.todaysDate(),
    ...
    df_output = df_output.join(
        pd.read_sql_query(
            query, con, parse_dates=['PricingDate', 'Maturity'],
            index_col=id_type))
    settlement_date = CALENDAR.advance(valuation_date,
                                       ql.Period(SETTLEMENT_DAYS, ql.Days))
...
def evaluate_bonds(df_input, yield_curve_fit,
    ...
    for b in df_output.index:
        ...
        df_output.loc[b, 'Th Price'] = bond.NPV() - bond.accruedAmount(
            settlement_date)
...
def zero_df_curve(yield_curve, tenors,
    ...
    sample_rates = [yield_curve.zeroRate(d, day_count, compounding).rate()
