...
class MBONOCurveFit():
    ...
    def __init__(self, market_data, date = ql.Date.todaysDate(), 
        ...
        self.curve, self.fit = clean_fit(mbono_market_snapshot(market_data,
                            valuation_date = date, id_type = id_type), 
                                         valuation_date = date, 
                                         min_days = min_days, 
                                         outlier_range = outlier_range)
    
    def get_val_data (self, market_data, id_type):
        val_data = evaluate_bonds(mbono_market_snapshot(market_data,
                            valuation_date = self.date, 
                            id_type = self.id_type), self.curve, 
                                  valuation_date = self.date)
        return val_data
